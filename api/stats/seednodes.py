...
def remove_notary(notary):
    params = {"mmrpc": "2.0", "params": {"name": notary}}
...
def empty_pg_table():
    ...
    CONN.commit()
    print(
        "Deleted", CURSOR.rowcount, "records from seednode_version_stats PgSQL table."
    )
...
def get_version_stats_from_sqlite_db(from_time=0, version=None):
    ...
    if not version:
        sql = (
            f"SELECT * FROM stats_nodes WHERE version != '' and timestamp > {from_time}"
        )
    print(sql)
...
def migrate_sqlite_to_pgsql(ts):
    ...
    for row in rows:
        ...
        if row["version"] != "":
            ...
            score = get_version_score(
                row["version"], hr_timestamp, row["name"], wss_detected
            )
            row_data = (
                row["name"],
                season,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # import data from other server
            ...
            print(
                "invalid param, must be in [empty, start, nodes, register, migrate, sqlite_data, pgsql_data, wss_test, import]"
            )

    else:
        print(
            "no param given, must be in [empty, start, nodes, register, migrate, sqlite_data, pgsql_data, wss_test, import]"
        )
