...
class OrderbookItem(BaseModel):
    ...
    bids: list[list[str, str]] = [["0.00337853", "11.00000000"]]
    asks: list[list[str, str]] = [["0.00344367", "53.00000000"]]
...
async def gecko_pairs(days: int = 7):
    try:
...
@router.get(
    ...
    description="Provides last 24-hour price and volume information on each market pair traded on exchange in last 7 days."
)
def gecko_tickers():
    try:
...
def gecko_historical_trades(
    ...
    try:
        ...
        logger.warning(f"{type(e)} Error in /api/v3/gecko/historical_trades [{ticker_id}]: {e}")
